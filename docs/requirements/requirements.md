...
- **문서 상태**: 초안 (v0.1, 2026-07-23) — 미결 15건이 **D-001~D-014**로, PRD 작성 중 모호점 3건이 **D-015~D-017**로, PRD 기술 검증 결과가 **D-018~D-036**으로 확정됐다(아래 5.5절). 열린 이슈는 외부 입력이 필요한 **I-016~I-019** 4건이다.
- **파생 문서**: [`../prd/PRD.md`](../prd/PRD.md) — 이 문서에서 파생된 개발용 PRD(v0.1 39건). **충돌 시 이 문서가 이긴다.**
...
| **정족수** | 대상자의 **1/3 이상**이 투표(찬성+반대+기권)해야 유효. 미달 시 `closed_invalid`. **정족수 = `ceil(대상자 수 / 3)`**(D-032 — `floor`면 대상자 2명일 때 0이 되어 아무도 투표하지 않아도 유효해진다) | 정족수 없음 / 과반 참여 / `floor` | **D-003**·**D-032** |
...
| **집계 공개 범위** | 집계만 공개하고 개인 선택은 비공개. 단 **대상자가 5명 미만이면 진행 중 집계를 숨기고 "N명 참여"만 노출**한 뒤 종료 시 공개한다(D-031 — 2~3명이면 집계에서 자기 표를 빼는 것만으로 타인의 선택이 드러나며, RLS로는 막을 수 없다) | 항상 공개 / 기명 투표 | **D-003**·**D-031** |
| **종료 트리거** | ① 기한 도래 자동 종료 ② 제안자·임원·오너의 수동 조기 종료 ③ 미투표자가 0명이 되면 즉시 자동 종료. **여기서 미투표자 = `스냅샷 명단 ∩ 현재 투표 가능자`**(D-022 — 탈퇴자를 분자에 남기면 미투표자가 영원히 0에 닿지 않아 이 트리거가 죽는다). 실행 수단은 **Supabase Cron(pg_cron)**, Vercel Cron 아님(D-027) | ①만 / ③ 삭제 | **D-003**·**D-022**·**D-027** |
...
**스냅샷의 저장 형태**: 대상자 명단은 배열 컬럼이 아니라 **`poll_eligible_voter` 조인 테이블**에 둔다(**D-025**). D-015(강퇴자 제외 발송)·NFR-029(재시도 횟수)·FR-042(미투표 수)·D-022(현재 투표 가능 여부)가 전부 **대상자별 상태**를 요구하는데 배열에는 담을 자리가 없기 때문이다.
...
| 팔레트 | 사전 검증된 **N = 12색**. 각 색은 라이트/다크 배경 각각에 대해 **대비 ≥ 3:1** 을 미리 측정해 통과한 값만 등록한다. **단일 팔레트 한 벌로 두 모드를 모두 통과시킬 수 있다**(D-026 — 휘도 창 L∈[0.109, 0.300]에서 12색상이 양쪽 배경에 4.45:1). 따라서 라이트/다크 쌍을 12조 정의하는 것은 **선택 사항**이다. `globals.css`의 `@theme inline`에 둔다 (R-005 참조) |
| 충돌 처리 | 같은 **날짜 셀 안에서** 색이 겹치면, 팔레트의 **색각 이상 ΔE 최대화 순서**에서 다음 색을 임시 배정한다(**D-026** — 기존의 "인접하지 않은 다음 인덱스" 규칙은 팔레트 인덱스 거리가 지각적 거리와 무관해 무효로 판명됐다). 전역 유일성은 보장하지 않는다(불가능하므로 요구하지 않는다). D-014로 크루 수 상한이 없어 **소속 크루 13개부터 중복이 불가피**하므로 FR-061의 크루 필터가 필수다(**R-017**) |
| 색 이외의 구분 | 바에 **크루명 텍스트 라벨**(공간 부족 시 말줄임)과 **`aria-label`에 크루명 + Meetup 제목**을 반드시 넣는다. 색은 보조 신호일 뿐이며 **이 항목은 불가침이다**(D-026) — 3:1 × 2모드 제약이 색을 좁은 휘도 창에 가둬 등휘도화시키므로, **2형 색각에서 66쌍 중 6쌍이 ΔE<10으로 붕괴**한다(최소 2.0). 색만으로 구분되지 않는 구간이 반드시 남는다 |
...
- **예외 흐름**:
  ...
  - E4 인증 메일 미수신 → 재발송(60초 쿨다운, 시간당 5회 상한). **이 상한은 커스텀 SMTP를 전제한다**(D-021 — Supabase 내장 이메일은 시간당 2통이며 그것도 프로젝트 전체 기준이라 동시 가입자 3명이면 3번째부터 발송되지 않는다). 공급자 선정은 **I-016**
...
- **수용 기준**:
  ...
  - AC5 Given 같은 시각에 3명이 가입, When 각자 인증 메일을 기다림, Then 3명 모두에게 발송된다(D-021의 커스텀 SMTP 전제를 검증하는 기준).
- **출처**: 원문 №1
...
- **예외 흐름**: E1 자격 증명 불일치 → 어느 필드가 틀렸는지 구분하지 않는 단일 메시지 / E2 5회 연속 실패 → 15분 잠금(**계정 단위**) / E3 세션 만료 → 재로그인 유도 후 원래 경로 복귀 / E4 미인증 계정 → FR-001 E4로 이관.
- **수용 기준**:
...
- **확정(D-020)**: E2·AC4의 **계정 단위 잠금은 Supabase Auth에 없다** — 기본 레이트 리밋은 IP 또는 프로젝트 단위라 ① 같은 IP의 다른 사용자를 함께 막고 ② 분산 IP 공격을 못 막으며 ③ "자격 증명이 맞아도 거부"라는 AC4의 의미론을 표현할 수 없다. 따라서 `AuthAttempt` 테이블 + 로그인 경로 게이트로 **자체 구현**한다. v0.1(Mock)에서는 잠금 **화면 상태**만 만든다.
...
- **사전조건**: 인증된 회원.
- **정상 흐름**: ① 개설 화면 → ② 크루명·소개·카테고리·공개 범위 입력(**색상은 묻지 않는다** — D-016) → ③ 제출 → ④ 크루 생성 + 개설자 멤버십 `active`/`owner` 생성 + 게시판·채팅방 자동 생성 + **색상 자동 배정**(FR-062) → ⑤ 크루 홈으로 이동.
...
- **확정(D-016)**: 색은 개설 시 자동 배정하고, 변경은 **크루 설정(FR-011·SC-15)** 에서만 한다. 개설 폼의 입력 항목은 크루명·소개·카테고리·공개 범위 넷이다.
- **출처**: 원문 №2
...
- **수용 기준**:
  - AC1 Given `private` 크루, When 비소속 회원이 크루 탐색 화면의 **어떤 경로로든**(키워드 검색·카테고리 필터·검색어 없는 전체 목록·추천 목록) 조회, Then 어느 경로에서도 나타나지 않는다(**D-017**).
...
- **정상 흐름**: ① 검색 화면 → ② 키워드 입력 또는 카테고리 필터(**입력 없이 전체 목록 브라우징도 가능**) → ③ 결과 목록(크루명, 소개 요약, 멤버 수, 카테고리, 색) → ④ 상세 진입 → ⑤ 가입 신청(FR-022). **모든 조회 경로에서 `private` 크루는 비소속자에게 제외된다**(D-017).
...
- **수용 기준**:
  ...
  - AC1-1 Given `private` 크루가 속한 카테고리, When 비소속자가 **검색어 없이** 그 카테고리를 필터링, Then 해당 `private` 크루는 목록에 없다(D-017).
  - AC1-2 Given 비소속자, When 크루 목록 API를 직접 호출, Then 응답에 `private` 크루가 애초에 포함되지 않는다(화면 필터링이 아니라 데이터 접근 규칙으로 구현 — NFR-011·NFR-012).
...
- **수용 기준**:
  ...
  - AC4 Given 대상자가 4명인 진행 중 투표, When 상세 조회, Then 찬성·반대 수가 아니라 **"N명 참여"만** 표시되고, 종료 후에 최종 집계가 공개된다(D-031).
- **확정(D-003·D-031)**: **집계만 공개, 개인 선택 비공개.** 단 **대상자 5명 미만**이면 진행 중 집계를 숨긴다 — 2~3명 크루에서는 집계에서 자기 표를 빼는 것만으로 타인의 선택이 드러나며, `PollVote` 행을 RLS로 막아도 **집계 자체가 누설**하므로 접근 제어로는 풀 수 없다.
- **출처**: 보완
...
- **수용 기준**:
  ...
  - AC2 Given **현재 투표 가능한** 대상자가 전원 투표 완료, When 마지막 투표 제출, Then 마감 전이라도 즉시 종료된다. **투표 도중 탈퇴한 대상자는 이 판정의 분자에서 제외한다**(D-022 — 스냅샷에 남겨 두면 미투표자가 영원히 0에 닿지 않아 이 트리거가 죽는다). 정족수 분모는 D-003대로 스냅샷 전체다.
  - AC3 Given 제안자, When 조기 종료 실행, Then 확인 절차를 거쳐 종료되고 종료 주체가 기록된다.
  - AC4 Given 자동 종료 작업이 5분 지연, When 사용자가 상세 조회, Then 마감 시각이 지난 투표는 **"결과 집계 중"** 으로 보인다 — 열람 시점 fallback은 **표시만** 복구하고 Meetup 생성(FR-060)·알림 적재(FR-045)는 복구하지 않으므로, 확정 전까지 캘린더에 나타나지 않는다(**D-024**).
- **확정(D-027)**: 자동 종료의 실행 수단은 **Supabase Cron(pg_cron)** 이다. Vercel Cron은 쓰지 않는다 — Hobby는 하루 1회·±59분이며 더 잦은 표현식은 배포 자체가 실패한다.
- **출처**: 원문 №8 ("투표가 종료 후")
...
- **확정(D-032)**: 정족수 = **`ceil(대상자 수 / 3)`**. 대상자가 3의 배수가 아닌 모든 경우의 판정이 여기서 갈린다.
- **출처**: 원문 №9 (판정 기준은 보완 — **D-003**·**D-032**로 확정)
...
- **예외 흐름**: E1 알림 끈 사용자 → 알림 센터에는 적재하되 토스트는 생략 / E2 발송 실패 → 재시도 3회 후 실패 기록 / E3 **탈퇴자·강퇴자 → 발송 제외**(D-015). 표의 유효성 판정은 별개로 D-003을 따른다(탈퇴자 표 유효, 강퇴자 표 무효).
- **수용 기준**:
  - AC1 Given 대상자 스냅샷 10명 중 1명이 강퇴됨, When 투표 종료, Then 알림 레코드는 **9건**이 생성된다(미투표자는 포함, 강퇴자는 제외 — D-015).
  - AC1-1 Given 이탈자 없는 대상자 10명, When 투표 종료, Then 알림 레코드 10건이 생성된다(미투표자 포함).
  - AC2 Given 접속 중인 대상자, When **판정이 완료**, Then **5초 이내** 토스트가 표시된다. **t=0은 판정 완료 시각이며 마감 시각이 아니다**(D-024) — 마감→판정 지연은 스케줄 주기(D-027)에 종속되므로 별도 목표치로 분리해 관리한다.
  - AC3 Given 미접속 대상자, When 다음 로그인, Then 알림 센터에 읽지 않음 상태로 남아 있다.
  - AC4 Given 알림 클릭, When 이동, Then 해당 제안글 상세로 이동한다.
- **확정(D-015)**: 강퇴자에게는 발송하지 않는다.
- **출처**: 원문 №8 (push는 범위 밖 — FR-073, D-004)
...
- **정상 흐름**: ① 바/날짜 클릭 → ② 패널(데스크톱: 사이드 / 모바일: 바텀시트) 열림 → ③ 해당 날짜의 Meetup 목록(크루명, 제목, 시각, 참여 인원, 색) → ④ 항목 클릭 시 원 제안글로 이동.
  ...
  > **정정(2026-07-29, 23일차 — D-070)**: ②의 괄호 "(데스크톱: 사이드 / 모바일: 바텀시트)"는
  > 이 문서 전체에서 "데스크톱"이 언급되는 유일한 자리였다. day-21 앱 프레임 도입(D-066,
  > `docs/prioritization-and-risks.md`)이 "넓은 화면에서도 항상 모바일 폭 프레임 하나만
  > 보여준다"고 확정하면서 이 괄호와 정면으로 상충하게 됐다(BOARD가 I-099 교차검증 중 발견).
  > 사용자 판단(23일차)으로 이 괄호를 **비구속**으로 확정한다 — ① 아래 AC1~AC4 어디에도
  > 데스크톱/모바일 구분이 없다(패널 형태는 수용 기준 대상이 아니었다), ② 이 구분을 요구하는
  > PRD 근거가 없다, ③ 원문 №10을 확정한 **D-012**(3.7절 오타 해석)는 패널이 "표시된다"는
  > 것만 다뤘지 형태(사이드/바텀시트)는 다룬 적이 없다. 이 괄호는 구현 문서(`AppShell.tsx`
  > 옛 docstring이 NFR-026에 "전환"을 스스로 추가했던 것과 같은 종류로, D-070이 원문에 없던
  > 조건을 문서가 스스로 덧붙인 것으로 판정한다. **정정된 흐름**: "② 패널(항상 프레임 안
  > 하단 바텀시트) 열림". 원 문구는 지우지 않고 이력으로 남긴다. 상세·구현 수정·실측:
  > `docs/decisions/appframe-responsive-audit-099.md` §7, 이슈 `I-104`.
...
- **정상 흐름**: ① Meetup 상세 또는 캘린더 패널 진입 → ② "참석" 클릭 → ③ **조건부 UPDATE**(`update meetup set attendingCount = attendingCount + 1 where id = ? and attendingCount < capacity`)로 정원 확인과 증가를 한 번에 수행 → ④ 참석 확정 → ⑤ `MeetupAttendance` 행 생성.
...
- **확정(D-019)**: 원자성은 **RLS로 보장하지 않는다.** RLS `WITH CHECK`의 count 서브쿼리는 Read Committed에서 동시 트랜잭션 간 격리되지 않아 정원 초과가 조용히 발생한다. `Meetup.attendingCount` **조건부 UPDATE**(행 락 획득 후 최신 행으로 WHERE 재평가) + `check (attendingCount <= capacity)` + **`unique(meetupId, profileId)`** 로 보장한다. 유일 제약은 FR-067 E2 멱등성의 전제이기도 하다.
- **출처**: 보완 (**D-013**의 귀결)
...
**엔티티 22종.** 그중 `DevicePushToken`은 차기 릴리스용이라 이번에는 테이블을 만들지 않고 타입 자리만 확보한다(D-004).

2026-07-23 기술 검증으로 세 가지가 바뀌었다 — `Poll.eligibleSnapshot` 배열을 **`PollEligibleVoter` 조인 테이블**로 교체(**D-025**), `Poll.quorumRatio` 제거(**D-032** — D-003이 1/3 고정이므로 컬럼이 설정 가능성을 잘못 암시했다), **`AuthAttempt`** 추가(**D-020**).
...
| `Poll` | id, postId, opensAt, closesAt, status, closedBy, result, decidedAt | 1:1 Post, 1:N PollEligibleVoter | 크루 멤버 읽기. 상태 전이는 서버 로직 전용(클라이언트 직접 수정 금지) |
| `PollEligibleVoter` *(D-025)* | pollId, profileId, notifiedAt, notifyAttempts, PK(pollId, profileId) | N:1 Poll/Profile | 같은 크루 `active` 멤버만 조회. 쓰기는 서버 로직 전용 |
...
| `Meetup` | id, crewId, pollId, title, date, **startTime?, place?, capacity?**, **attendingCount**, description, status(**confirmed**/cancelled), createdAt | N:1 Crew/Poll, 1:N MeetupAttendance | 해당 크루 `active` 멤버만. 생성은 서버 로직 전용, 취소·변경은 제안자·임원+ |
| `MeetupAttendance` *(D-013)* | meetupId, profileId, status(attending/absent), respondedAt, **UNIQUE(meetupId, profileId)** | N:1 Meetup/Profile | 같은 크루 `active` 멤버만 조회. **본인 행만** insert/update. **정원 판정은 RLS가 아니라 `Meetup.attendingCount`의 조건부 UPDATE로** 원자성을 보장한다(**D-019**) |
...
| `AuthAttempt` *(D-020)* | identifier, attemptedAt, succeeded | — | **클라이언트 접근 불가.** 로그인 경로(Server Action/Edge Function)에서만 읽고 쓴다. Supabase Auth에 계정 단위 잠금 기능이 없어 자체 구현한다 |
...
| ID | 요구사항 | 측정 기준 |
| --- | --- | --- |
| NFR-001 | 주요 화면 초기 렌더 성능. **달성 수단은 메모이제이션이 아니라 렌더링 전략**이다(**D-029**) — 긴 목록 윈도잉, 안정적인 `key`, `startTransition`, 사전 인덱싱. React Compiler는 **컴포넌트와 훅만** 메모이즈하므로 채팅·캘린더의 실제 INP 병목에는 관여하지 않는다 | 모바일 4G 시뮬레이션, **LCP p75 ≤ 2.5s**, **INP p75 ≤ 200ms**, **CLS p75 ≤ 0.1** |
...
| ID | 요구사항 | 측정 기준 |
| --- | --- | --- |
| NFR-007 | 채팅·투표·알림은 Supabase Realtime **Broadcast**로 갱신한다(**D-023** — Postgres Changes는 "변경 1건 × 구독자 N명 = 인가 검사 N회"이며 단일 스레드라 컴퓨트 증설로 개선되지 않는다). DB 트리거 + `realtime.broadcast_changes()`를 쓰고, 구독은 **사용자당 1연결로 다중화**한다. 폴링을 기본 수단으로 쓰지 않는다 | 구현 검토로 확인 |
...
| NFR-011 | 모든 테이블에 RLS를 켜고 **기본 거부(deny by default)** 로 시작한다. **`realtime.messages`의 Realtime Authorization 정책도 이 범위에 포함한다**(D-023). 크루 종속 테이블의 정책은 **비노출 스키마의 `SECURITY DEFINER` 헬퍼**를 거쳐 자기참조 재귀를 끊고, 모든 정책에 `TO` 절을 명시한다(**D-028**) | 정책 없는 테이블 0개 |
...
| NFR-033 | 채팅 메시지는 작성 **12개월 후 배치로 자동 파기**하고, 크루 해산 시 함께 파기한다(**D-009**). 구현은 **배치 `DELETE` 루프**이며 `pg_partman` 파티셔닝은 채택하지 않는다(**D-033** — 파티셔닝은 테이블 생성 시점에 결정해야 하고 나중에 바꾸려면 재생성이 필요하다. 삭제량이 배치 창을 넘으면 v0.2 스키마 설계에서 재검토). 개인정보 처리방침에 보관 기간 12개월을 고지한다 | 13개월 전 메시지가 조회되지 않는다. 파기 배치가 v0.2에 존재하고 잡당 10분을 넘지 않는다(CON-10) |
...
| CON-03 | React Compiler가 켜져 있으므로 수동 `useMemo`/`useCallback`/`memo`를 쓰지 않는다. **단 측정 근거를 기록하면 `"use no memo"` 또는 수동 메모이제이션을 허용한다**(**D-029** — React 공식 문서가 이들을 "탈출구로 계속 사용할 수 있다"고 명시한다. 근거 없는 예외는 불가) | R-004, **D-029** |
| CON-04 | 디자인 토큰은 `src/app/globals.css`의 `@theme inline`에 추가한다. **캘린더 팔레트는 단일 한 벌로 라이트·다크 양쪽을 통과시킬 수 있으므로 12조 쌍 정의는 선택 사항이다**(**D-026**). 팔레트 12색도 여기에 정의 | R-005, FR-062 |
...
| CON-08 | Realtime 채널 수·동시 연결이 Supabase 요금제 한도에 걸리지 않는지 설계 시 확인한다. 실측 한도는 동시 연결 Free 200 / Pro 500 / Pro(지출 상한 해제) 10,000 이며 **NFR-006의 1,000세션은 앞의 둘을 초과**한다 | R-011, **I-017**, **R-019** |
| CON-09 | 모든 신규 컴포넌트는 `/sample`에 4상태(기본·로딩·빈 상태·오류)로 등록한다. **"오류"에는 네트워크 실패뿐 아니라 도메인 오류(RLS 403·정원 마감·동시 수정 충돌)를 포함한다**(**D-030**) | `CLAUDE.md`, KPI-6, **D-030** |
| CON-10 | 주기 실행(투표 자동 종료·채팅 파기)은 **Supabase Cron(pg_cron)** 으로 한다. Vercel Cron은 쓰지 않는다. 동시 잡 8개·잡당 10분 이내를 지킨다 | **D-027**, D-003, D-009 |
| CON-11 | 인증 메일 발송은 **커스텀 SMTP**를 전제한다. 내장 이메일(시간당 2통·프로젝트 전체)로는 FR-001 E4를 만족할 수 없다 | **D-021**, I-016 |
| CON-12 | Mock↔실데이터 경계 4개를 v0.1부터 지킨다 — 표현/컨테이너 분리 · `subscribeToRoom(id, onEvent): Unsubscribe` 구독 추상화 · 4상태 "오류"에 도메인 오류 포함 · 인증 경계는 레이아웃에서 처리. Server Action + `refresh()` 패턴을 Mock 단계부터 쓴다 | **D-030**, NFR-034·037 |
...
| **D-015** | 투표 종료 알림을 **강퇴자에게는 발송하지 않는다**(탈퇴자와 동일 취급). 표의 유효성 판정은 D-003 그대로 | — (PRD 작성 중 발견) |
| **D-016** | 크루 색상은 **개설 시 자동 배정**하고 개설 폼에서 묻지 않는다. 변경은 크루 설정(SC-15)에서만 | — (PRD 작성 중 발견) |
| **D-017** | `private` 크루는 **모든 조회 경로**(검색·카테고리·전체 목록·추천)에서 비소속자에게 비노출. 데이터 접근 규칙으로 구현 | — (PRD 작성 중 발견) |
| **D-018** | mo_im **전용 Supabase 프로젝트**를 쓴다(기존 ref는 타 앱이 점유) | — (기술 검증) |
| **D-019** | 정원 원자성 = `attendingCount` 조건부 UPDATE + `unique`. RLS로 시도하지 않음 | — |
| **D-020** | 계정 잠금은 **자체 구현**(`AuthAttempt` + 로그인 게이트) | — |
| **D-021** | **커스텀 SMTP 필수** 외부 의존성 | — |
| **D-022** | 트리거 ③의 미투표자 = **스냅샷 ∩ 현재 투표 가능자** | — |
| **D-023** | Realtime은 **Broadcast** 채택 + `realtime.messages` 인가 정책 | — |
| **D-024** | FR-045 AC2의 **t=0 = 판정 완료 시각** | — |
| **D-025** | 대상자 명단을 **`PollEligibleVoter` 조인 테이블**로 | — |
| **D-026** | 팔레트 순서는 **색각 이상 ΔE 최대화**, 단일 팔레트로 양 모드 커버 (D-006 충돌 회피 규칙 일부 대체) | — |
| **D-027** | 스케줄 = **Supabase Cron(pg_cron)**, Vercel Cron 아님 | — |
| **D-028** | RLS 헬퍼를 **비노출 스키마의 `SECURITY DEFINER`** 로 | — |
| **D-029** | React Compiler 금지에 **예외 절차**(측정 근거 + `"use no memo"`) | — |
| **D-030** | Mock↔실데이터 **경계 4개**를 v0.1부터 규약화 | — |
| **D-031** | **대상자 5명 미만**이면 진행 중 집계 은닉, 종료 후 공개 | — |
| **D-032** | 정족수 = **`ceil(대상자/3)`**, `quorumRatio` 컬럼 제거 | — |
| **D-033** | 채팅 파기는 **배치 루프**, 파티셔닝 미채택 | — |
| **D-034** | `Meetup.status` = **`confirmed`**/`cancelled` | — |
| **D-035** | PRD §7 누락 필드 복구 | — |
| **D-036** | PRD §8은 **설치됨/도입 예정** 구분 | — |

#### 열린 미결 — 외부 입력이 필요하다 (상세는 [`../ISSUES.md`](../ISSUES.md))

| I-\* | 질문 | 막히는 것 |
| --- | --- | --- |
| **I-016** | 커스텀 SMTP 공급자를 무엇으로 할 것인가 | FR-001 E4 (D-021) |
| **I-017** | Supabase 요금제 — NFR-006의 1,000세션은 Pro+지출상한 해제 이상을 전제한다 | NFR-006, D-023 |
| **I-018** | Realtime 팬아웃 계수 실측 (브로드캐스트를 1로 세는가 N으로 세는가) | 용량 계획 전체, R-019 |
| **I-019** | 팀 규모 — v0.1 168.5인일을 기간으로 환산할 수 없다 | 일정 산출, R-020 |
...
| **크루 목록 RLS** | D-017 | 노출 판정을 화면별 필터가 아니라 **테이블 단위 정책**으로 설계해야 한다(NFR-011·NFR-012) |
| **Supabase 프로젝트 신설** | D-018 | 기존 ref는 타 앱이 점유 중이다. `.mcp.json`·`CLAUDE.md`·PRD §8 **세 곳**의 ref를 한 번에 교체해야 한다(R-018) |
| **`AuthAttempt` 자체 구현** | D-020 | Supabase Auth에 계정 단위 잠금이 없어 로그인 게이트를 직접 만든다. **v0.2 공수에 없던 항목** |
| **커스텀 SMTP 계약** | D-021 | 도메인 인증(SPF/DKIM) 리드타임이 있어 조기 착수가 필요하다. 공급자는 I-016 |
| **`PollEligibleVoter` 조인 테이블** | D-025 | 배열 컬럼으로는 대상자별 상태를 담을 수 없다. NFR-032가 소급 변경을 금지하므로 **스키마 설계가 마지막 기회** |
| **RLS 헬퍼 스키마** | D-028 | 비노출 스키마에 `SECURITY DEFINER` 함수를 두어 자기참조 재귀를 끊는다. 크루 종속 **전 테이블**에 걸린다 |
| **팔레트 색값 확정** | D-026 | 색각 이상 ΔE를 측정해 순서를 정한 뒤 `globals.css`에 넣는다 |
| **`CLAUDE.md` 갱신 (2차)** | D-018·D-029·D-030 | Supabase ref, React Compiler 예외 절차, Mock 경계 4개 |
...
| **R-018** | ref 교체를 놓친 채 마이그레이션을 적용하면 타 앱 운영 DB를 오염시킨다 (**D-018의 귀결**) |
| **R-019** | Realtime 팬아웃 계수·채널 한도 미확인으로 용량 계획이 성립하지 않는다 |
| **R-020** | v0.1 공수가 168.5인일(1인 약 10개월)로 기대와 어긋날 위험 |
...
| FR-064 | Meetup 상세 조회 | 보완 |
| FR-065 | Meetup 취소·일정 변경 | 보완 |
| FR-066 | 참석/불참 응답 (선착순·정원 마감) | 보완 (D-013) |
| FR-067 | 참석 취소 시 자리 반환 | 보완 (D-013) |
| FR-068 | 참석자 목록 조회 | 보완 (D-013) |
| FR-070 | 인앱 토스트 알림 | №8 |
| FR-071 | 알림 센터 | 보완 |
...
**합계: FR 54건 (원문에 근거가 있는 것 25건 / 보완 29건), NFR 41건.** 6.1절 우선순위 표의 54건과 일치한다.
