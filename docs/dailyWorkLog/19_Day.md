...
1. **I-065는 20일차 작업으로 확정됐다** — 회차 마감에 사용자가 두 갈래 중 **"익명 흐름 전용 리밋 정책을 정해 구현"** 을 택했다("잔여 위험 수용·종결"은 기각). **순서가 중요하다**: 숫자·윈도·IP 신뢰경계를 **먼저 새 결정 D-\*로 등재**하고 그 다음 구현한다 — 구현은 기존 자산(`evaluateFixedWindowRateLimit`·카운터 테이블 패턴·`headers()` 전례) 재사용으로 저비용이고, 어려운 부분은 숫자의 근거다(D-005의 20회/60초는 **계정당·인증** 위협모델 값이라 그대로 옮길 수 없다). zone 제약도 CORE가 미리 조사해 뒀다 — 카운터 조회·기록은 zone 3, 정리 잡은 BOARD 소유 함수를 건드리지 않고 신설.
