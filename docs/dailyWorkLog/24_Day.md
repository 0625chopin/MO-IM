...
이 회차는 시작과 끝에 각각 사용자 판단을 받았다. 둘 다 팀이 단독으로 정할 수 없는 층위였다.

1. **테스트 러너(vitest)를 최소 스펙으로 도입한다**(회차 시작 시). D-052가 "043B가 마지막 판단 자리"라고 명시적으로 다음 판단 시점을 남겨 뒀고, 그 043B가 이번 회차에 진행되므로 팀장이 사용자 확인을 받았다. 선택지는 ① 최소 스펙 도입 ② 미루고 R-002를 "수용"으로 재분류 ③ SQL·TS 대조까지 포함이었고, **①이 선택됐다.** → **D-072**로 승격.
2. **초대 만료(I-030)는 상태 전이가 아니라 조회 필터링으로 다룬다**(회차 종료 시). CREW가 후보 A/B/C를 정리하고 B-1을 권장했는데, **DESIGN이 교차검증에서 네 번째 축**(배치 없이 `listInvitationsForProfile`에 `expires_at > now()` 추가)을 찾아냈고 **그것이 채택됐다.** → **D-073**으로 승격. 근거는 "만료 자체는 이미 DB 레벨에서 강제된다" — 같은 회차의 I-114 수정으로 만료된 초대로는 가입이 성립하지 않으므로 **`invited` 행이 남아도 권한상 무해**하고 남은 건 표시 문제뿐이었다. **구현은 다음 회차 CREW 몫**(팀원 종료 후 결정이 나왔다). 알려진 한계 2건은 D-073에 있다.
...
1. **I-030 구현이 곧바로 착수 가능하다 — CREW 몫.** 회차 종료 시 사용자 결정으로 **D-073**이 확정됐다(후보 D: 조회 쿼리 필터링). 대상은 `listInvitationsForProfile`에 `expires_at > now()` 추가와 `requirements.md` §2.4에 "만료는 상태 전이가 아니라 조회 필터링으로 다룬다(D-073)" 주석이다. **결정만 하고 구현하지 않은 이유**는 팀원 4명이 종료된 뒤 사용자 결정이 나왔기 때문이다. 한계 2건(‘invited’ 행 잔류로 집계에 유령 인원 가능 · §2.4 화살표 공백은 의도로 명시)은 D-073 참고.
...
- 브랜치: `day-24`
- 커밋: `a130e09` — 34개 파일, +3713/-176. 사용자 결정(D-073) 반영분은 후속 커밋.
- 푸시: **`origin/day-24` 성공**(사용자 승인)
