...
export const ko = {
  ...
  home: {
    ...
    /**
     * D-109 — 메인(랜딩 `/` + 홈 `/home`) "지금 활발한 모임" 목록. `dashboard.upcoming`
     * ("다가오는 모임")과 **다른 것이다**: 저건 내가 속한 크루의 일정이고, 이건 공개 크루
     * 전체에서 최근 활동이 많은 순으로 고른 소개용 목록이다. 두 섹션이 같은 화면(`/home`)에
     * 함께 놓이므로 제목이 서로 헷갈리지 않게 "다가오는"과 "활발한"으로 갈랐다.
     *
     * **"핫한"이라는 낱말을 쓰지 않았다** — 제품 문체가 담백한 한국어(따뜻한 종이·잉크)라
     * 유행어가 겉돈다. `subtitle`이 기준(최근 7일 활동)을 밝혀 "왜 이게 여기 있는지"를
     * 사용자가 알 수 있게 했다 — 근거 없는 랭킹은 신뢰를 깎는다.
     */
    hotMeetups: {
      title: "지금 활발한 모임",
      subtitle: "최근 일주일 동안 이야기가 활발했던 공개 크루의 예정된 모임이에요",
      /** 게스트(랜딩)에게 보이는 유도 문구 — 홈에서는 쓰지 않는다. */
      guestCta: "크루 둘러보기",
      /** 순위 배지. `{rank}`는 1~5. */
      rankBadge: "{rank}위",
      attending: "참석 {count}명",
      attendingWithCapacity: "참석 {count}/{capacity}명",
      /** 시작 시각이 정해지지 않은 모임. */
      timeUndecided: "시간 미정",
      empty: "아직 공개된 예정 모임이 없어요",
      emptyDescription: "공개 크루에 예정된 모임이 생기면 여기에 보여드릴게요.",
      errorTitle: "활발한 모임을 불러오지 못했어요",
      errorDescription: "잠시 후 다시 시도해 주세요.",
    },
  },
